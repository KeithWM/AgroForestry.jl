...
showname(plant::PlantSpecs.Plant; kwargs...) = showname(plant.latin; kwargs...)
showname(::Missing; kwargs...) = "MISSING"
function showname(latin::AbstractString; sep="\n")
    return join(map(x -> "$(lowercase(x[1:3])).", split(latin, " ")[1:2]), sep)
end
